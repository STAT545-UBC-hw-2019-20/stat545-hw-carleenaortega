...
gapminder %>% 
  ...
  labs(x="Continent",y=",ax GDP ($)",title="The Max GDP of continents")+
  theme_bw()
  
```

By default, the continents are arranged alphabetically. We can use the forcats package (fct_rev() command) to arrange the continents in an ascending order of max GDP.

```{r}
gapminder %>% 
  ggplot()+
  geom_bar(aes(fct_reorder(continent,gdpPercap,max)),colour="royal blue", fill="royal blue")+
  labs(x="Continent",y="Max GDP ($)",title="The Max GDP of continents")+
  theme_bw()
  
```
This is an easier way of showing 
