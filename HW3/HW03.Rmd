...
mutate(min=min(gdpPercap),max=max(gdpPercap)) #if this is chosen instead of summarize, can plot but table shows multiple entries per continent
  summarize(min=min(gdpPercap),max=max(gdpPercap)) #if this is chosen instead of mutate, cannot plot the next. but allows a summarized table
knitr::kable(t2) %>% 
...
mutate(min=min(gdpPercap),max=max(gdpPercap)) #if this is chosen instead of summarize, can plot but table shows multiple entries per continent
  summarize(min=min(gdpPercap),max=max(gdpPercap)) #if this is chosen instead of mutate, cannot plot the next. but allows a summarized table
  ggplot(t2)+
  geom_line(aes(continent,gdpPercap)) +
