...
# try making bar graph for each year in each continent
```
...
email2<-separate_rows(email,guest,sep=",")
email2$guest <- factor(email2$guest)
email2[,1] <- trimws(email2[,1])
guest_email2=tibble::as_tibble(guest) %>% 
  left_join(email2,by=c("name"="guest"))
knitr::kable(guest_email2) %>% 
...
```{r}
emails_nonlist<-left_join(email2,guest,by=c("guest"="name"))

knitr::kable(emails_nonlist) %>% 
kable_styling("striped", latex_options = "scale_down",full_width = FALSE)
  
```
```{r}
emails_nonlist2 <- emails_nonlist %>% dplyr::filter(is.na(party))
```
### 3.3 Make a guestlist
Make a guestlist that includes everyone we have emails for (in addition to those on the original guestlist).

```{r}
merged_list=tibble::as_tibble(guest) %>% 
  full_join(email2,by=c("name"="guest"))
knitr::kable(merged_list) %>% 
kable_styling("striped", latex_options = "scale_down",full_width = FALSE)
```
