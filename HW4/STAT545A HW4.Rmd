...
email_sep<-separate_rows(email,guest,sep=",")

email_sep[,1] <- trimws(email_sep[,1])

email_sep$guest <- factor(email_sep$guest)  

guest_email=tibble::as_tibble(guest) %>% 
  left_join(email_sep,by=c("name"="guest"))
knitr::kable(guest_email) %>% 
...
```{r}
left_join(email_sep,guest,by=c("guest"="name")) %>% 
filter(is.na(party)) %>% 
knitr::kable() %>% 
kable_styling("striped", latex_options = "scale_down",full_width = FALSE)
```
